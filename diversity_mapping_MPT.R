...
setwd("C:\\Users\\AnaCarnaval\\Dropbox\\Andrea_Lab\\trasnfer_819_PC\\Functional_chap\\OR_AUC_MPT//")
models<-list.files(pattern="AF_")
Stack_models<-stack(models)
species_names_models<-sub("_OR_AUC_MPT","",models) 
...
plot(TD)
writeRaster(TD,"richness_hylids_complete_0_0083_MPT.asc")
...
write.table(community_data1, file = "communities_hylidsMPT.txt", append = FALSE,row.names=F,quote=F,sep="\t") #tratar de agregar nombre de mascara
...
write.table(community_data1, file = "communities_hylids_and_pd_MPT.txt", append = FALSE,row.names=F,quote=F,sep="\t")
...
writeRaster(pd_ras,"hylids_PD_MPT.asc",format="ascii")
